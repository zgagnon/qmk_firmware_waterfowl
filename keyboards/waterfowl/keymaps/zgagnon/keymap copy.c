...
[_COLEMAK] = LAYOUT(
	...
	KC_1,				TG(_SYMNUM),		KC_BSPC,			TD(TD_SHIFT_ESC),			KC_ESC,		LGUI(LCTL(LSFT(KC_4))),	KC_ENT,				LT(_SPOON, KC_SPC),				LT(_RECT, KC_TAB),		KC_4
),
 ...
 * |      |      | sx_u |      |      |                      |->col |      |foc_u |      |      |
 * |------+------+------+------+------|                      |------+------+------+------+------|
 * |      | sx_l | sx_d | sx_r |      |                      |col-> |foc_l |foc_d |foc_r |      |
 * |------+------+------+------+------|  ,-----.    ,-----.  |------+------+------+------+------|
 * | Reset|      |      |      |      |  |CAPS |    |NUMLK|  | Cen  | full | ->w  | ->h  |      |
 * `----------------------------------'  `-----'    `-----'  `----------------------------------'
 *          ,-----.   ,--------------------.            ,--------------------.   ,-----.
 *          |  1  |   | L1 | SPACE | ESC  |            |  ENT  | BS | ENTER |   |  4  |
 *          `-----'   `--------------------'            `--------------------'   `-----'
 */
[_SPOON] =  LAYOUT(
	KC_NO,	KC_NO,		                    LGUI(LCTL(LALT(LSFT(KC_UP)))),		KC_NO,	                        KC_NO,		LGUI(LCTL(LALT(KC_I))),	                KC_NO,	                    LGUI(LCTL(LALT(KC_UP))),	KC_NO,	                    KC_NO,
	KC_NO,	LGUI(LCTL(LALT(LSFT(KC_UP)))),	LGUI(LCTL(LALT(LSFT(KC_UP)))),		LGUI(LCTL(LALT(LSFT(KC_UP)))),	KC_NO,		LGUI(LCTL(LALT(KC_O))),	                LGUI(LCTL(LALT(KC_LEFT))),	LGUI(LCTL(LALT(KC_DOWN))),	LGUI(LCTL(LALT(KC_RIGHT))),	KC_NO,
	KC_NO,	KC_NO,		                    KC_NO,		                        KC_NO,	                        KC_NO,		LGUI(LCTL(LALT(KC_C))),	LGUI(LCTL(LALT(KC_F))),	    LGUI(LCTL(LALT(KC_R))),	    LGUI(LCTL(LALT(LSFT(KC_R)))),	    KC_NO,
	KC_1,	TO(_COLEMAK),	LT(1,KC_SPC),	KC_ESC,	KC_CAPS,		KC_NUM,	KC_ENT,	KC_BSPC,	LT(2,KC_ENT),	KC_4
),
};
